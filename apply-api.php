<?php
/**
 * Direct Professional Application Endpoint
 * Bypasses Laravel middleware for public form submissions
 */
...
try {
    ...
    if ($rate < 500) {
        ...
        exit(json_encode([
            'success' => false,
            'message' => 'Validation failed',
            'errors' => ['rate_per_hour' => ['The rate per hour must be at least 500']]
        ]));
    }
    ...
    if (!empty($_FILES['professional_photo']['tmp_name'])) {
        ...
        $storageDir = __DIR__ . '/api/storage/uploads/professionals/photos';
    ...
    if (!empty($_FILES['license_document']['tmp_name'])) {
        ...
        $storageDir = __DIR__ . '/api/storage/uploads/professionals/licenses';
        @mkdir($storageDir, 0775, true);
        $timestamp = date('YmdHis');
        $email = str_replace('@', '_', $_POST['email']);
        $filename = "{$email}_{$timestamp}.{$ext}";
